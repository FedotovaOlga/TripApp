...
@Component({
  ...
  imports: [MatFormFieldModule, MatInputModule, FormField, MatCheckboxModule, MatAnchor],
...
export class TripAdd {
  ...
  onSubmit() {
    submit(this.tripForm, async () => {
      const tripData = {
        ...this.trip(),
        startAt: new Date(this.trip().startAt).toISOString(),
        endAt: new Date(this.trip().endAt).toISOString()
      };
